const roomData = {
    ...
    std_mod: { title_key: "r2_title", price: 1950, img: "https://images.unsplash.com/photo-1590490360182-c33d57733427?q=80&w=800", desc_key: "r2_desc" },
    std_cozy: { title_key: "r3_title", price: 2100, img: "https://assets.hyatt.com/content/dam/hyatt/hyattdam/images/2025/05/08/1000/SINSS-P0040-Cozy-Single.jpg/SINSS-P0040-Cozy-Single.16x9.jpg", desc_key: "r3_desc" },
    family: { title_key: "r4_title", price: 3800, img: "https://images.unsplash.com/photo-1560067174-c5a3a8f37060?q=80&w=800", desc_key: "r4_desc" },
    ...
    jun_prem: { title_key: "r6_title", price: 3500, img: "https://images.unsplash.com/photo-1578683010236-d716f9a3f461?q=80&w=800", desc_key: "r6_desc" },
    ...
    lux_royal: { title_key: "r8_title", price: 6200, img: "https://images.unsplash.com/photo-1591088398332-8a7791972843?q=80&w=800", desc_key: "r8_desc" },
    lux_pano: { title_key: "r9_title", price: 6800, img: "https://cf.bstatic.com/xdata/images/hotel/max1024x768/206566089.jpg?k=ef239d7e021c1990939095bf8a743484f28a87213cd4747afa2c50aebb9539b5&o=", desc_key: "r9_desc" },
    presidential: { title_key: "r10_title", price: 8500, img: "https://images.unsplash.com/photo-1600566753190-17f0baa2a6c3?q=80&w=800", desc_key: "r10_desc" }
};

const offersData = {
    early: { title_key: "off_early_title", img: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?q=80&w=800", glow: "gold", desc_key: "off_early_full" },
    romance: { title_key: "off_romance_title", img: "https://images.unsplash.com/photo-1529290130-4ca3753253ae?q=80&w=800", glow: "pink", desc_key: "off_romance_full" }
};
...
const heroImages = [
    "url('https://media.joinup.travel/storage/hotel/50541/photos/Panorama-De-Lyuks-3.jpg')",
    "url('https://images.unsplash.com/photo-1520250497591-112f2f40a3f4?q=80&w=1600')",
    "url('https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?q=80&w=1600')"
];
let currentImage = 0;
...
function startSession(email, name) {
    ...
    localStorage.setItem('hotelUserName', name || '');
    updateAuthUI();
}
...
async function renderOrders() {
    ...
    if(user && typeof db !== 'undefined') {
        ordersList.innerHTML = `<p style="color:#999;">⏳ Завантаження...</p>`;
        try {
            ...
            ordersList.innerHTML = sortedDocs.map(doc => {
                const o = doc.data();
                const date = o.createdAt ? new Date(o.createdAt.seconds * 1000).toLocaleDateString('uk-UA') : '';
                return `
                <div class="order-item">
                    <h4>${o.room}</h4>
                    <p style="font-size: 14px; color: #555; margin-top: 5px;">
                        ${translations[lang].sum_dates}: <strong>${o.checkin} — ${o.checkout}</strong><br>
                        ${o.total > 0 ? `${translations[lang].sum_total}: <strong style="color: #c5a059;">${o.total.toLocaleString()} ₴</strong><br>` : ''}
                        ${date ? `<span style="color:#bbb; font-size:12px;">Оформлено: ${date}</span>` : ''}
                    </p>
                </div>`;
            }).join('');
        } catch(err) {
            console.error('Firestore error:', err);
            ordersList.innerHTML = `<p style="color:#c00;">Помилка завантаження. Перевірте налаштування Firebase.</p>`;
        }
    } else {
        ordersList.innerHTML = `<p style="color: #666;" data-i18n="dash_no_orders">${translations[lang].dash_no_orders}</p>`;
    }
...
async function renderReviews() {
    const myReviewsList = document.getElementById('reviews-list');
    const publicReviewsTrack = document.getElementById('reviews-track');
    if(!myReviewsList && !publicReviewsTrack) return;

    const user = localStorage.getItem('hotelUser');

    // В кабінеті — відгуки поточного користувача
    if(myReviewsList) {
        if(user && typeof db !== 'undefined') {
            myReviewsList.innerHTML = `<p style="color:#999;">⏳ Завантаження...</p>`;
            try {
                ...
                if(snap.empty) {
                    myReviewsList.innerHTML = `<p style="color: #666;">Ви ще не залишили жодного відгуку.</p>`;
                } else {
                    const sortedReviewDocs = snap.docs.slice().sort((a, b) => {
                        const ta = a.data().createdAt ? a.data().createdAt.seconds : 0;
                        const tb = b.data().createdAt ? b.data().createdAt.seconds : 0;
                        return tb - ta;
                    });
                    myReviewsList.innerHTML = sortedReviewDocs.map(doc => {
                        const r = doc.data();
                        const date = r.createdAt ? new Date(r.createdAt.seconds * 1000).toLocaleDateString('uk-UA') : '';
                        return `
                        <div class="review-item">
                            <div class="stars" style="color: #fbbc04; margin-bottom: 5px;">${'<i class="fas fa-star"></i>'.repeat(r.score)}${'<i class="far fa-star"></i>'.repeat(5-r.score)}</div>
                            <p style="color: #555; font-style: italic;">"${r.text}"</p>
                            <p style="font-size: 12px; color: #999; margin-top: 5px;">${date}</p>
                        </div>`;
                    }).join('');
                }
            } catch(err) {
                console.error('Firestore error:', err);
                myReviewsList.innerHTML = `<p style="color:#c00;">Помилка завантаження відгуків.</p>`;
            }
        } else {
            myReviewsList.innerHTML = `<p style="color: #666;">Ви ще не залишили жодного відгуку.</p>`;
        }
    ...
    // На головному сайті — публічні відгуки (останні N від усіх користувачів)
    if(publicReviewsTrack && typeof db !== 'undefined') {
        document.querySelectorAll('.user-generated-review').forEach(r => r.remove());
        try {
            const snap = await db.collection('reviews').orderBy('createdAt', 'desc').limit(10).get();
            snap.docs.forEach(doc => {
                const r = doc.data();
                const userName = r.userName || (r.userEmail || 'Гість').split('@')[0];
                const initial = userName.charAt(0).toUpperCase();
                const div = document.createElement('div');
                div.className = 'review-card user-generated-review';
                div.innerHTML = `
                    <div class="rev-header">
                        <div class="rev-avatar" style="background:#c5a059;">${initial}</div>
                        <div class="rev-info"><h4>${userName}</h4><div class="stars">${'<i class="fas fa-star"></i>'.repeat(r.score)}${'<i class="far fa-star"></i>'.repeat(5-r.score)}</div></div>
                    </div>
                    <p>${r.text}</p>
                `;
                publicReviewsTrack.prepend(div);
            });
        } catch(err) {
            console.warn('Firestore public reviews error:', err);
        }
    }
}
...
if(btnLogin) btnLogin.onclick = () => { resetAuthTabs(); modalAuth.style.display = 'flex'; };
if(mobileLogin) mobileLogin.onclick = (e) => { e.preventDefault(); resetAuthTabs(); modalAuth.style.display = 'flex'; document.getElementById('mobile-menu').classList.remove('active'); setTimeout(() => document.getElementById('mobile-menu').style.display = 'none', 300); };
...
document.querySelectorAll('.auth-tab').forEach(tab => {
    tab.onclick = () => {
        ...
        const target = tab.dataset.authTab === 'register' ? document.getElementById('register-form') : document.getElementById('login-form');
...
if(loginForm) {
    loginForm.onsubmit = async (e) => {
        ...
        if(typeof db === 'undefined') {
            showToast(lang === 'uk' ? "Немає з'єднання з базою даних." : "No database connection.");
            return;
        }

        try {
            const userRef = db.collection('users').doc(sanitizeEmailId(email));
            const docSnap = await userRef.get();
            if(!docSnap.exists) {
                showToast(lang === 'uk' ? "Користувача з таким email не знайдено. Зареєструйтесь." : "No user found with this email. Please register.");
                return;
            }
            const userData = docSnap.data();
            if(userData.password && userData.password !== password) {
                showToast(lang === 'uk' ? "Невірний email або пароль." : "Invalid email or password.");
                return;
            }
            await userRef.set({ password: userData.password || password, lastLogin: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true });
            startSession(email, userData.name || '');
            showToast(lang === 'uk' ? "Ви успішно увійшли!" : "You have successfully logged in!");
            modalAuth.style.display = 'none';
            loginForm.reset();
            renderReviews();
        } catch(err) {
            console.error('Login error:', err);
            showToast(lang === 'uk' ? "Помилка входу. Спробуйте ще раз." : "Login error. Please try again.");
        }
    };
}
...
if(registerForm) {
    registerForm.onsubmit = async (e) => {
        ...
        if(password !== password2) {
            showToast(lang === 'uk' ? "Паролі не співпадають." : "Passwords do not match.");
            return;
        }
        if(password.length < 4) {
            showToast(lang === 'uk' ? "Пароль має містити щонайменше 4 символи." : "Password must be at least 4 characters.");
            return;
        }
        if(typeof db === 'undefined') {
            showToast(lang === 'uk' ? "Немає з'єднання з базою даних." : "No database connection.");
        ...
        try {
            ...
            const docSnap = await userRef.get();
            if(docSnap.exists) {
                showToast(lang === 'uk' ? "Користувач з таким email вже зареєстрований." : "A user with this email already exists.");
                return;
            }
            await userRef.set({
                name: name,
            ...
            showToast(lang === 'uk' ? "Реєстрація успішна! Вітаємо." : "Registration successful! Welcome.");
            modalAuth.style.display = 'none';
            registerForm.reset();
            renderReviews();
        } catch(err) {
            console.error('Register error:', err);
            showToast(lang === 'uk' ? "Помилка реєстрації. Спробуйте ще раз." : "Registration error. Please try again.");
        }
    };
}
...
if(btnSaveName) {
    btnSaveName.onclick = async () => {
        ...
        const email = localStorage.getItem('hotelUser');
        if(!email) return;
        if(!newName) {
            showToast(lang === 'uk' ? "Введіть, будь ласка, ваше ім'я." : "Please enter your name.");
            return;
        }
        if(typeof db !== 'undefined') {
            try {
                await db.collection('users').doc(sanitizeEmailId(email)).set({ name: newName }, { merge: true });
            } catch(err) { console.warn('Firestore name update error:', err); }
        }
        localStorage.setItem('hotelUserName', newName);
        document.getElementById('user-name-display').innerText = newName;
        showToast(lang === 'uk' ? "Ім'я успішно оновлено!" : "Name updated successfully!");
    };
}
...
if(ratingStars && reviewScoreInput) {
    ratingStars.forEach(star => {
        star.onclick = () => {
            const val = parseInt(star.dataset.val);
            reviewScoreInput.value = val;
...
if(addReviewForm) {
    addReviewForm.onsubmit = async (e) => {
        ...
        if(score === 0) {
            showToast(lang === 'uk' ? "Будь ласка, оберіть оцінку!" : "Please select a rating!");
            return;
        }
        const text = document.getElementById('review-text').value;
        const userEmail = localStorage.getItem('hotelUser') || null;
        const userName  = localStorage.getItem('hotelUserName') || null;
        ...
        if(typeof db !== 'undefined') {
            ...
            } catch(err) { console.warn('Firestore review error:', err); }
        }

        showToast(lang === 'uk' ? "Дякуємо за відгук!" : "Thank you for your review!");
        addReviewForm.reset();
...
// --- ВАЛІДАЦІЯ ТА ВІДПРАВКА БРОНЮВАННЯ ---
...
if(contactForm) {
    contactForm.onsubmit = async (e) => {
        ...
        const userEmail  = localStorage.getItem('hotelUser') || null;
        const nameVal    = document.getElementById('u-name')?.value.trim() || '';
        const surnameVal = document.getElementById('u-surname')?.value.trim() || '';
        const emailVal   = document.getElementById('u-email')?.value.trim() || '';
        const notesVal   = document.getElementById('u-notes')?.value.trim() || '';
...
// --- ВАЛІДАЦІЯ ТА ВІДПРАВКА ФОРМИ ЗВОРОТНОГО ЗВ'ЯЗКУ ---
...
if(feedbackForm) {
    feedbackForm.onsubmit = async (e) => {
        ...
        const nameVal = feedbackForm.querySelector('input[type="text"]')?.value.trim() || '';
        const msgVal  = feedbackForm.querySelector('textarea')?.value.trim() || '';
        ...
        if(typeof db !== 'undefined') {
            ...
            } catch(err) { console.warn('Firestore feedback error:', err); }
        }
...
const translations = {
    uk: {
        ...
        auth_title: "Авторизація", auth_pass: "Пароль", auth_tab_login: "Вхід", auth_tab_register: "Реєстрація", auth_name: "Ім'я", auth_pass_confirm: "Підтвердіть пароль", btn_save_name: "Зберегти",
        ...
        write_review_title: "Залишити відгук", review_rating: "Оцінка", review_placeholder: "Напишіть ваші враження тут...", btn_submit_review: "Опублікувати", my_reviews_title: "Ваші попередні відгуки",
    ...
    en: {
        ...
        auth_title: "Authorization", auth_pass: "Password", auth_tab_login: "Log In", auth_tab_register: "Register", auth_name: "Name", auth_pass_confirm: "Confirm Password", btn_save_name: "Save",
        ...
        write_review_title: "Leave a review", review_rating: "Rating", review_placeholder: "Write your impressions here...", btn_submit_review: "Publish", my_reviews_title: "Your previous reviews",
